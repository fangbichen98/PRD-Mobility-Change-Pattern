...
def collect(outputs_dir: Path) -> pd.DataFrame:
    rows = []
    for run_dir in sorted(outputs_dir.glob("multiscale_temporal_*_phase2_dir34_random_*")):
...
def main():
    ...
    out_csv = outputs_dir / "dir34_random_fast_summary.csv"
    out_md = outputs_dir / "dir34_random_fast_summary.md"

    df = collect(outputs_dir)
    if df.empty:
        out_csv.write_text("", encoding="utf-8")
        out_md.write_text("# Direction3/4 Random Fast Summary\n\nNo completed runs found.\n", encoding="utf-8")
        print("No completed runs found.")
        return

    df.to_csv(out_csv, index=False)

    lines = ["# Direction3/4 Random Fast Summary", ""]
    lines.append("| Run | Mode | Seed | Node Feature | Test Acc (%) | Test F1 | Edges(2021/2024) |")
    lines.append("|---|---|---:|---|---:|---:|---|")
    for _, r in df.iterrows():
        lines.append(
            f"| {r['run_dir']} | {r['mode']} | {int(r['seed']) if pd.notna(r['seed']) else -1} | {r['node_feature_mode']} | "
