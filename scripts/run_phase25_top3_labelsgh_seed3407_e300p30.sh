...
SEED="${SEED:-3407}"
LOG_DIR="outputs/phase25_top3_labelsgh_seed${SEED}_logs"
mkdir -p "$LOG_DIR"

RESULTS_CSV="$LOG_DIR/phase25_results.csv"
RESULTS_MD="$LOG_DIR/phase25_results_table.md"
...
run_one() {
  ...
  echo "[PHASE25] start model=${model_label} temporal=${temporal_model} seed=${SEED}" | tee -a "$log_file"
  ...
  if [[ $exit_code -ne 0 || -z "$run_dir" || "$run_dir" == "$run_dir_before" || ! -f "$run_dir/metrics/test_results.json" ]]; then
    echo "[PHASE25] failed model=${model_label} exit=${exit_code} run_dir=${run_dir:-N/A}" | tee -a "$log_file"
...
PY
  ...
  echo "[PHASE25] done model=${model_label} acc=${acc} f1=${f1} run_dir=${run_dir}" | tee -a "$log_file"
  echo "${model_label},${acc},${f1},${run_dir},OK" >> "$RESULTS_CSV"
}

run_one "LSTM+GCN" "LSTM" "phase25_lstm_gcn_label_sgh_spc250_seed${SEED}_e300p30"
run_one "轻量Transformer+GCN" "TRANSFORMER" "phase25_transformer_gcn_label_sgh_spc250_seed${SEED}_e300p30"
run_one "TCN+GCN" "TCN" "phase25_tcn_gcn_label_sgh_spc250_seed${SEED}_e300p30"
...
lines = []
lines.append('# Phase25 Top3 复跑结果（label_sgh, seed=${SEED}）')
lines.append('')
...
echo "[PHASE25] all runs completed."
echo "[PHASE25] CSV: ${RESULTS_CSV}"
echo "[PHASE25] Table: ${RESULTS_MD}"